...
Now that we have identified a set of differentially methylated regions we can proceed to creating the HTML report. Note that this report has less information than the [DiffBind case](/regionReportSupp/DiffBind.html) because we don't have a p-value variable.
...
dir.create('bumphunter-example', showWarnings = FALSE, recursive = TRUE)
...
report <- renderReport(regions, 'Example bumphunter', pvalueVars = NULL,
    ...
    outdir = 'bumphunter-example', output = 'index')
```



You can view the final report at [bumphunter-example](/regionReportSupp/bumphunter-example/index.html).
