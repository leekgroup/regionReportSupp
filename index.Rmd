...
[regionReport](http://bioconductor.org/packages/regionReport) can use `edgeR` results to create reports that look nearly identical to the reports from `DESeq2` results. The file [edgeR.html](/regionReportSupp/edgeR.html) shows how to create a HTML and a PDF report using results from `edgeR`. The resulting HTML report is available at [edgeR-example](/regionReportSupp/edgeR-example/index.html) and the PDF version at [edgerReport](/regionReportSupp/edgeR-example/edgeReport.pdf). 
